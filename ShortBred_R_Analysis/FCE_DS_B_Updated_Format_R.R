...
myT=myT[which((myT$Sample=="FCE")|(myT$Sample=="DS B")),]

library(stringi)

pValuesLocation <- vector()
pValuesFCE_DS_B <- vector()
...
for( i in 47:ncol(myT)){
  ...
  {
    FCE_DS_B=factor(myT$Sample,c("FCE","DS B"))
    myLm <- lm( myT[,i] ~ myT$Location + FCE_DS_B + factor(myT$Timepoint)  )		
    myAnova <- anova(myLm)
    pValuesLocation[index] <- myAnova$"Pr(>F)"[1]
    pValuesFCE_DS_B[index] <- myAnova$"Pr(>F)"[2]
...
dFrame <- data.frame(names, indexes,absolute_indexes, pValuesFCE_DS_B, pValuesLocation,pValuesTimepoint)
dFrame$pValuesFCE_DS_BAdjusted<- p.adjust( dFrame$pValuesFCE_DS_B, method = "BH")
...
dFrame <- dFrame [order(dFrame$pValuesFCE_DS_BAdjusted),]

write.table(dFrame, file=paste("shortbred_models_Updated_Format_FCE_DS_B", ".txt",sep=""), sep="\t",row.names=FALSE)

pdf("shortbredPlots_Updated_Format_FCE_DS_B.pdf")
for (j in 1:nrow(dFrame)){
  ...
  myFrame <- data.frame( bug, time, location, FCE_DS_B )
  boxplot(myT[,absolute_index] ~ FCE_DS_B, main = paste("Source ",format(dFrame$pValuesFCE_DS_BAdjusted[j])), names=c("FCE","DS_B"))
  stripchart(bug ~ FCE_DS_B,method="jitter",data = myFrame,vertical = TRUE, pch = 20, add=TRUE ) 
...
hist(pValuesFCE_DS_B)
hist(pValuesLocation)
hist(pValuesTimepoint)
plot.new()
hist(dFrame$pValuesFCE_DS_BAdjusted,main='pValuesFCE_DS_BAdjusted')
