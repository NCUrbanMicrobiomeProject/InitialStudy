...
dFrame <- data.frame(names, indexes,absolute_indexes,pValuesUpDown, pValuesLocation,pValuesTimepoint)
...
dFrame <- dFrame [order(dFrame$pValuesUpDownAdjusted),]

write.table(dFrame, file=paste("shortbred_models_Format_Updated_UpDown", ".txt",sep=""), sep="\t",row.names=FALSE)

pdf("shortbredPlots_Updated_Format_UpDown.pdf")
for (j in 1:nrow(dFrame)){
  ...
  boxplot(myT[,absolute_index] ~ updown, main = paste("Source ",format(dFrame$pValuesUpDownAdjusted[j])), names=c("Upstream","Downstream"))
  stripchart(bug ~ updown,method="jitter",data = myFrame,vertical = TRUE, pch = 20, add=TRUE ) 
...
plot.new()
hist(dFrame$pValuesUpDownAdjusted,main='pValuesUpDownAdjusted')
