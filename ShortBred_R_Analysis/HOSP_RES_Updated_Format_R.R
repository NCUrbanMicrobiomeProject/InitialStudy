...
myT=myT[which((myT$Sample=="HOSP")|(myT$Sample=="RES")),]

pValuesLocation <- vector()
pValuesHOSP_RES <- vector()
...
for( i in 47:ncol(myT)){
  ...
  {
    HOSP_RES=factor(myT$Sample,c("HOSP","RES"))
    myLm <- lm( myT[,i] ~ myT$Location + HOSP_RES + factor(myT$Timepoint)  )		
    myAnova <- anova(myLm)
    pValuesLocation[index] <- myAnova$"Pr(>F)"[1]
    pValuesHOSP_RES[index] <- myAnova$"Pr(>F)"[2]
...
dFrame <- data.frame(names, indexes,absolute_indexes,pValuesHOSP_RES, pValuesLocation,pValuesTimepoint)
dFrame$pValuesHOSP_RESAdjusted<- p.adjust( dFrame$pValuesHOSP_RES, method = "BH")
...
dFrame <- dFrame [order(dFrame$pValuesHOSP_RESAdjusted),]

write.table(dFrame, file=paste("shortbred_models_Updated_Format_HOSP_RES", ".txt",sep=""), sep="\t",row.names=FALSE)

pdf("shortbredPlots_Updated_Format_HOSP_RES.pdf")
for (j in 1:nrow(dFrame)){
  ...
  myFrame <- data.frame( bug, time, location, HOSP_RES )
  boxplot(myT[,absolute_index] ~ HOSP_RES, main = paste("Source ",format(dFrame$pValuesHOSP_RESAdjusted[j])), names=c("HOSP","RES"))
  stripchart(bug ~ HOSP_RES,method="jitter",data = myFrame,vertical = TRUE, pch = 20, add=TRUE ) 
...
hist(pValuesHOSP_RES)
hist(pValuesLocation)
hist(pValuesTimepoint)
plot.new()
hist(dFrame$pValuesHOSP_RESAdjusted,main='pValuesHOSP_RESAdjusted')
