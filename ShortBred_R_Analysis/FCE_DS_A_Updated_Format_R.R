...
myT=myT[which((myT$Sample=="FCE")|(myT$Sample=="DS A")),]

library(stringi)

pValuesLocation <- vector()
pValuesFCE_DS_A <- vector()
...
for( i in 47:ncol(myT)){
  ...
  {
    FCE_DS_A=factor(myT$Sample,c("FCE","DS A"))
    myLm <- lm( myT[,i] ~ myT$Location + FCE_DS_A + factor(myT$Timepoint)  )		
    myAnova <- anova(myLm)
    pValuesLocation[index] <- myAnova$"Pr(>F)"[1]
    pValuesFCE_DS_A[index] <- myAnova$"Pr(>F)"[2]
...
dFrame <- data.frame(names, indexes,absolute_indexes, pValuesFCE_DS_A, pValuesLocation,pValuesTimepoint)
dFrame$pValuesFCE_DS_AAdjusted<- p.adjust( dFrame$pValuesFCE_DS_A, method = "BH")
...
dFrame <- dFrame [order(dFrame$pValuesFCE_DS_AAdjusted),]

write.table(dFrame, file=paste("shortbred_models_Updated_Format_FCE_DS_A", ".txt",sep=""), sep="\t",row.names=FALSE)

pdf("shortbredPlots_Updated_Format_FCE_DS_A.pdf")
for (j in 1:nrow(dFrame)){
  ...
  myFrame <- data.frame( bug, time, location, FCE_DS_A )
  boxplot(myT[,absolute_index] ~ FCE_DS_A, main = paste("Source ",format(dFrame$pValuesFCE_DS_AAdjusted[j])), names=c("FCE","DS_A"))
  stripchart(bug ~ FCE_DS_A,method="jitter",data = myFrame,vertical = TRUE, pch = 20, add=TRUE ) 
...
hist(pValuesFCE_DS_A)
hist(pValuesLocation)
hist(pValuesTimepoint)
plot.new()
hist(dFrame$pValuesFCE_DS_AAdjusted,main='pValuesFCE_DS_AAdjusted')
