...
myT <- read.table("shortBREDwMetadataOnlyDeepestReads.txt", header=TRUE,sep="\t",comment.char = "@")
myT <- myT[ myT$Location == "Sugar Creek" | myT$Location == "Mallard Creek" ,]
myT <- myT[ myT$Sample=="FCE" | myT$Sample=="DS A" | myT$Sample=="DS B",]
library(stringi)

pValuesLocation <- vector()
pValuesFCEDown <- vector()
...
for( i in 47:ncol(myT)){
  ...
  {
    FCEDown <- factor(ifelse( myT$Sample=="DS A" | myT$Sample=="DS B" , "DOWN", "FCE"))
    FCEDown <- factor(FCEDown, c("DOWN","FCE"))
    
    myLm <- lm( myT[,i] ~ myT$Location + FCEDown + factor(myT$Timepoint)  )		
    myAnova <- anova(myLm)
    pValuesLocation[index] <- myAnova$"Pr(>F)"[1]
    pValuesFCEDown[index] <- myAnova$"Pr(>F)"[2]
...
dFrame <- data.frame(names, indexes,absolute_indexes,pValuesFCEDown, pValuesLocation,pValuesTimepoint)
dFrame$pValuesFCEDownAdjusted<- p.adjust( dFrame$pValuesFCEDown, method = "BH")
...
dFrame <- dFrame [order(dFrame$pValuesFCEDownAdjusted),]

write.table(dFrame, file=paste("shortbred_models_Updated_Format_FCE_Down", ".txt",sep=""), sep="\t",row.names=FALSE)

pdf("shortbredPlots_Updated_Format_FCE_Down.pdf")
for (j in 1:nrow(dFrame)){
  ...
  myFrame <- data.frame( bug, time, location, FCEDown)
  boxplot(myT[,absolute_index] ~ FCEDown, main = paste("Source ",format(dFrame$pValuesFCEDownAdjusted[j])), names=c("DOWN","FCE"))
  stripchart(bug ~ FCEDown,method="jitter",data = myFrame,vertical = TRUE, pch = 20, add=TRUE ) 
...
plot.new()
hist(dFrame$pValuesFCEDownAdjusted,main='pValuesFCEDownAdjusted')
