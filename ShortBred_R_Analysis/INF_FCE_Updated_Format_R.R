...
myT=myT[which((myT$Sample=="INF")|(myT$Sample=="FCE")),]

pValuesLocation <- vector()
pValuesINF_FCE <- vector()
...
for( i in 47:ncol(myT)){
  ...
  {
    INF_FCE=factor(myT$Sample,c("INF","FCE"))
    myLm <- lm( myT[,i] ~ myT$Location + INF_FCE + factor(myT$Timepoint)  )		
    myAnova <- anova(myLm)
    pValuesLocation[index] <- myAnova$"Pr(>F)"[1]
    pValuesINF_FCE[index] <- myAnova$"Pr(>F)"[2]
...
dFrame <- data.frame(names, indexes,absolute_indexes,pValuesINF_FCE, pValuesLocation,pValuesTimepoint)
dFrame$pValuesINF_FCEAdjusted<- p.adjust( dFrame$pValuesINF_FCE, method = "BH")
...
dFrame <- dFrame [order(dFrame$pValuesINF_FCEAdjusted),]

write.table(dFrame, file=paste("shortbred_models_Updated_Format_INF_FCE", ".txt",sep=""), sep="\t",row.names=FALSE)

pdf("shortbredPlots_Updated_Format_INF_FCE.pdf")
for (j in 1:nrow(dFrame)){
  ...
  myFrame <- data.frame( bug, time, location, INF_FCE )
  boxplot(myT[,absolute_index] ~ INF_FCE, main = paste("Source ",format(dFrame$pValuesINF_FCEAdjusted[j])), names=c("INF","FCE"))
  stripchart(bug ~ INF_FCE,method="jitter",data = myFrame,vertical = TRUE, pch = 20, add=TRUE ) 
...
plot.new()
hist(dFrame$pValuesINF_FCEAdjusted,main='pValuesINF_FCEAdjusted')
