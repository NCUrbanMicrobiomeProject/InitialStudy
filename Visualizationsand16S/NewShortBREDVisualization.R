...
myT$Sites.Compared = factor(myT$Sites.Compared, levels(myT$Sites.Compared)[c(6, 9, 7, 8, 1, 4, 5, 3, 2)])
...
colorVector <- as.character(colorVector)
...
testplot <- ggplot(sigwithColor, aes(Sites.Compared, justGene)) +
  geom_tile(aes(fill = colorVector)) +
  scale_fill_manual(values = c("red", "green", "blue", "yellow", "magenta", "cyan"),
                    name = "Significant\nDifferences\nwith Term(s)",
                    labels = c("Timepoint", "Sample Site", "Waterway", "Waterway and\n Sample Site", "Waterway and\nTimepoint", "Sample Site\nand Timepoint")) +
  xlab("Sites Compared") +
  ylab("Gene") +
  ggtitle("ShortBRED Genes with Significant Differences Between Sites") +
  theme(axis.text.y = element_text(size = 5), axis.text.x = element_text(size = 8), plot.title = element_text(hjust = 0.5))

ggsave(paste0("ShortBREDsiggenechangesUpdate.pdf"), width=8, height=12, plot = testplot)

sigwithColor$colorVector <- as.factor(sigwithColor$colorVector)
sigwithColor$colorVector = factor(sigwithColor$colorVector, levels(sigwithColor$colorVector)[c(3, 2, 1, 4)])

recolortestplot <- ggplot(sigwithColor, aes(Sites.Compared, justGene)) +
  geom_tile(aes(fill = colorVector)) +
...
ggsave(paste0("ShortBREDsiggenechangesUpdateRECOLOR.pdf"), width=8, height=12, plot = recolortestplot)
