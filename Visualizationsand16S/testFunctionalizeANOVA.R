...
## Plotting all
## INPUT: tests data.frame, original data data.frame
##
## DECISIONS:
## Type of data
## OUTPUT: pdf

## Just plotting the significant
## Should just require sending it different data.frame

## Need to robustly read in regardless of the source

## Need a helper function for sample subsetting for the comparisons
## up versus down (requires pooling)
## hosp versus res
## hosp versus inf
## res versus inf
## by each paired across streams
## from each within WWTP
## from FCE to DS A
## from DS A to DS B

## Also want UP A B versus MNT A B versus UW A B
## But B is missing

## Running all of them
## How consistent are UP A UP B across timepoints?
rm(list=ls())
## library("phyloseq")
...
functionlm <- function(df, responseCols, explainString, rareCut) {
  ...
  ## print(pvaldf[1,])
  ## print(paste0("The do.call worked for ", paste0(locVec, collapse="_")))
  ...
  print(paste0("colnames worked for ", paste0(locVec, collapse="_")))
...
dataType <- "qiimeopenmerge"
## dataType <- "MetaPhlAn"
## dataType <- "HUMANn"
##dataType <- "shortBRED"
# dataType <- "antibiotics"
...
if(dataType == "qiimeopenmerge") {
  dataDir <- paste(baseDir, "qiime/open_reference_CSSnormalized_merged/", sep="/")
  isQIIME <- TRUE
  ...
  dataDir <- paste(baseDir, "MetaPhlAn/", sep="/")
  isQIIME <- FALSE
} else if (dataType == "HUMANn") {
  dataDir <- paste(baseDir, "HUMANn/UpdatedData", sep="/")
  isQIIME <- FALSE
} else if (dataType == "antibiotics") {
  dataDir <- baseDir
  isQIIME <- FALSE
}
...
for (taxa in taxonomicLevels) {
  ...
  if (dataType == "shortBRED") {
    ...
    fileName <- paste(dataType, "_CSSwithMetadata_L_",taxa,".txt", sep="")
  } else if (dataType == "MetaPhlAn") {
    if( taxa == 2) {
  ...
  for(locVec in runCompare) {
    ...
    if(any(locVec %in% elsewhere) == FALSE) {
      subT <- subT[myT$Location == "Mallard Creek" | myT$Location == "Sugar Creek",]
    }
    ## Not called currently
    if(length(locVec) == 3) {
      if (dataType != "qiimeopenmerge") {
        ## expRel <- c("Sample + InputReads + Timepoint")
        expRel <- c("Sample + Timepoint")
      } else {
        ## expRel <- c("Sample + sampleDepth + Timepoint")
        expRel <- c("Sample + Timepoint")
      }
      subT <- subT[subT$Location == locVec[3],]
    } else if (length(locVec) == 2){
      if (dataType != "qiimeopenmerge") {
        ## expRel <- c("Location + Sample + InputReads + Timepoint")
        expRel <- c("Location + Sample + Timepoint")
      } else {
        ## expRel <- c("Location + Sample + sampleDepth + Timepoint")
        expRel <- c("Location + Sample + Timepoint")
      }
      ...
      if (all.equal(c("UP", "RURAL"), locVec) == TRUE) {
        ...
        if (dataType != "qiimeopenmerge") {
          ## expRel <- c("Location + Sample + InputReads")
          expRel <- c("Sample")
          
        } else {
          ## expRel <- c("Location + Sample + sampleDepth")
          expRel <- c("Sample")
      ...
      ## Not sure if this will be running correctly anymore :important:
      if (dataType != "qiimeopenmerge") {
        ## expRel <- c("Location + InputReads + Timepoint")
        expRel <- c("Location + Timepoint")
      } else {
        ## expRel <- c("Location + sampleDepth + Timepoint")
        expRel <- c("Location + Timepoint")
      }
    }
    ## print(expRel)
    ...
    functionCompare(subT, locVec, responseCols, expRel, rareCut, pValThreshold, "Abundance")
  }
  
  ## Model on Imputed Antibiotics
  ## Dropping the Ertapenem and Amxocillin cases
  ## Need to change the naming convention
  ## This whole section needn't be run
  if (imputeValues > 0) {
    for(locVec in runCompare) {
      for(antibioticCol in c(19:26)){
        ...
        ## Should be any
        ## But does this matter as it is for the antibiotic columns
        if(any(locVec %in% elsewhere) == FALSE) {
          subT <- subT[myT$Location == "Mallard Creek" | myT$Location == "Sugar Creek",]
        }
        if(length(locVec) == 3) {
          if (dataType != "qiimeopenmerge") {
            ## expRel <- c("Sample + Antibiotic + InputReads + Timepoint")
            expRel <- c("Sample + Antibiotic + Timepoint")
          } else {
            ## expRel <- c("Sample + Antibiotic + sampleDepth + Timepoint")
            expRel <- c("Sample + Antibiotic +  Timepoint")
          }
          subT <- subT[subT$Location == locVec[3],]
        } else if (length(locVec) == 2){
          if (dataType != "qiimeopenmerge") {
            ## expRel <- c("Location + Sample + Antibiotic + InputReads + Timepoint")
            expRel <- c("Location + Sample + Antibiotic + Timepoint")
            
          } else {
            ## expRel <- c("Location + Sample + Antibiotic + sampleDepth + Timepoint")
            expRel <- c("Location + Sample + Antibiotic + Timepoint")
          }
          if (all.equal(c("UP", "DOWN"), locVec) == TRUE) {
            subT[subT$Sample == "UP A" | subT$Sample == "UP B",]$Sample <- "UP"
          ...
          ## Remind myself about what this means
          if (dataType != "qiimeopenmerge") {
            ## expRel <- c("Location + Antibiotic + InputReads + Timepoint")
            expRel <- c("Location + Antibiotic + Timepoint")
          } else {
            ## expRel <- c("Location + Antibiotic + sampleDepth + Timepoint")
            expRel <- c("Location + Antibiotic + Timepoint")
          }
        }
        subT$Location <- as.factor(subT$Location)
        ...
        ## Quick way to just out the antibiotic computations
        ## functionCompare(subT, locVec, responseCols, expRel, rareCut, pValThreshold, paste0("AbundanceVImpute",imputeValues,colnames(subT)[antibioticCol]))
      }
    }
  } else {
    for(locVec in runCompare) {
      for(antibioticCol in c(19:26)){
        ...
        ## The additional offset for the construction of the "Antibiotic" column
        responseCols <- c((endMetadataIndex+1):(ncol(myT)-3))
        ## Should be any
        ## But does this matter because of the antibioticCols
        if(any(locVec %in% elsewhere) == FALSE) {
        ...
        ## Quick way to cut out the antibiotic modeling
        ##functionCompare(subT, locVec, responseCols, expRel, rareCut, pValThreshold, paste0("AbundanceVNon-Impute",imputeValues, colnames(subT)[antibioticCol]))
      }
    }
  }
  ## This should happen limited number of times
  if(taxa == 2 & ( dataType == "shortBRED" | dataType == "HUMANn" | dataType == "qiimeopenmerge")) {
    responseCols <- c(19:26)
    ## The imputed antibiotics values doesn't really matter here because of the choice of variables
    if(imputeValues > 0) {
      for(locVec in runCompare) {
        subT <- myT
        ## Was all, should be any?
        ## But this section is broken anyhow?
        if(any(locVec %in% elsewhere) == FALSE) {
          subT <- subT[subT$Location == "Mallard Creek" | subT$Location == "Sugar Creek",]
        }
        if(length(locVec) == 3) {
          ...
          if (all.equal(c("UP", "RURAL"), locVec) == TRUE) {
            ## This shouldn't cause any kind of a problem here.
            subT <- myT
            subT[subT$Sample == "UP A" | subT$Sample == "UP B",]$Sample <- "UP"
            subT[subT$Sample == "MNT A" | subT$Sample == "MNT B" | subT$Sample == "UW A" | subT$Sample == "UW B",]$Sample <- "RURAL"
            if (dataType != "qiimeopenmerge") {
        ...
        ## print(paste0("It should get to here at ", paste0(locVec, collapse="_")))
        #functionCompare(subT, locVec, responseCols, expRel, rareCut, pValThreshold, paste0("ImputedAntibiotics",imputeValues))
      }
    } else {
      for(locVec in runCompare) {
        subT <- myT
        ## Need to keep only non NA rows :TODO:
        subT <- subT[complete.cases(subT[,responseCols]),]
        ...
        #functionCompare(subT, locVec, responseCols, expRel, rareCut, pValThreshold, "Non-ImputedAntibiotics")
      }
    }
  }
}
