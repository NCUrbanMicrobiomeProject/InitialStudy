...
fileName <- "./MetaPhlAn/analysis/metaphlan_species_filtered/TestOneFile.tsv"
...
sigwithColor$colorVector = factor(sigwithColor$colorVector, levels(sigwithColor$colorVector)[c(4, 5, 3, 1, 2, 6)])

testplot <- ggplot(sigwithColor, aes(Sites.Compared, Taxon)) +
  ...
  xlab("Sites Compared") +
  ylab("WGS Taxon") +
  ggtitle("WGS Taxa with Significant Differences Between Sites") +
...
ggsave(paste0("WGSsiggenechangesUpdate.pdf"), width =12, height = 50, limitsize=FALSE, plot = testplot)
